...
import os, logging, tempfile, datetime, shutil, time
...
"""
Codes below are copied from
https://github.com/jiacheng-xu/DiscoBERT with some minor modifications.
"""
...
@Metric.register("RougeScore")
class RougeScore(Metric):
    def __init__(self, name='rouge',cand_path='/tmp', ref_path='/tmp/', path_to_valid='/tmp'):
        super(RougeScore, self).__init__()
        self.pred_str_bag = []
        self.ref_str_bag = []
        self.id_bag = []
        self.name = name
        self.temp_dir = tempfile.mkdtemp(prefix=path_to_valid)
        self.cand_path = cand_path
        self.ref_path = ref_path

    @overrides
    def __call__(self, pred: str, ref: str, id: str, **kwargs):
        self.pred_str_bag.append(pred)
        self.ref_str_bag.append(ref)
        self.id_bag.append(id)
    ...
    def get_metric(self, reset: bool):
        ...
        assert len(self.ref_str_bag) == len(self.pred_str_bag)
